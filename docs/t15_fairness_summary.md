...
| fff_student_stage_h_no_balance_cosine | final_test | validation_leader_full_test | true | 0.914800 | 0.915133 |  |  | Three validation-trained Stage H seeds per family, full CIFAR-10 test evaluation after validation-only Stage H HPO. |
| fff_student_stage_h_baseline_cosine | final_test | all_family_full_test | true | 0.908000 | 0.909133 |  |  | Three validation-trained Stage H seeds per family, full CIFAR-10 test evaluation after validation-only Stage H HPO. |
| fff_student_stage_h_wsd | final_test | all_family_full_test | true | 0.903067 | 0.905700 |  |  | Three validation-trained Stage H seeds per family, full CIFAR-10 test evaluation after validation-only Stage H HPO. |
| fff_student_stage_h_low_lr_cosine | final_test | all_family_full_test | true | 0.889533 | 0.889133 |  |  | Three validation-trained Stage H seeds per family, full CIFAR-10 test evaluation after validation-only Stage H HPO. |
| official_fastfeedforward_fff | shape_budget_harness | shape_budget_regression_harness_available | false |  |  |  |  | CPU deterministic shape/budget/regression harness only; no CIFAR-10 validation run. |
| dense_teacher_copied_student | final_test | validation_selected_full_test | true | 0.931667 | 0.927867 |  |  | Three validation-selected seeds, three fine-tune epochs, 4218 train steps each, same teacher checkpoint, then full CIFAR-10 test evaluation. |
| matched_smaller_dense_linear | final_test | validation_selected_full_test | true | 0.931933 | 0.927500 |  |  | Three validation-selected seeds, three fine-tune epochs, 4218 train steps each, same teacher checkpoint, then full CIFAR-10 test evaluation. |
| matched_low_rank_linear | final_test | validation_selected_full_test | true | 0.926400 | 0.921200 |  |  | Three validation-selected seeds, three fine-tune epochs, 4218 train steps each, same teacher checkpoint, then full CIFAR-10 test evaluation. |
| shared_only_rows_baseline | final_test | validation_selected_full_test | true | 0.910000 | 0.906267 |  |  | Three validation-selected seeds, three fine-tune epochs, 4218 train steps each, same teacher checkpoint, then full CIFAR-10 test evaluation. |
...
- Completed T15 baseline rows are final-test rows only when backed by validation-selected full CIFAR-10 test artifacts; older validation-only baseline summaries are superseded for final accuracy claims.
