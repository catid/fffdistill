...
claim. Full final-test quality evidence is now backed by the Stage H
all-family final-test comparison below.
...
- Selection rule: `no_balance_cosine` was selected by the highest mean
  ...
  any full CIFAR-10 test evaluation; the follow-up all-family final-test wave
  evaluated every completed Stage H validation family/seed without using test
  metrics for selection.
- Selection manifest: `docs/stage_h_all_families_final_selection_manifest.jsonl`.
- Summary: `docs/stage_h_all_families_final_test_summary.md`.
- Validation run: `stage_h_validation_full3ep_20260704_5b5e4e1`.
- Final-eval run: `student_final_stage_h_all_families_20260704_10a7691`
  at final-eval commit `10a7691f16b4b11cb8eabad54037c957f2a42f58`.
...
- Full Stage H all-family FFF student final CIFAR-10 validation-leader validation mean: `0.914800`.
- Full Stage H all-family FFF student final CIFAR-10 validation-leader test mean accuracy: `0.915133`.
- Full Stage H all-family FFF student final CIFAR-10 validation-leader test std accuracy: `0.003421`.
- Stage H all-family final-test mean accuracies: no_balance_cosine `0.915133`,
  baseline_cosine `0.909133`, wsd `0.905700`, low_lr_cosine `0.889133`.
- Best Stage H all-family final-test case: `no_balance_cosine_seed21002`,
  seed `21002`, test accuracy `0.919000`.
- All twelve final-test rows have `partial_test_evaluation=false`,
  `allow_untracked_selection=false`, and `test_accessed=true`; their selection
  records have `test_accessed=false`. The three `low_lr_cosine` rows are marked
  below target in the family summary and were run with the explicit
  `allow_below_target=true` failure-analysis override.
...
The table contains 48 rows and enforces that `test_accessed=true` appears only on
`final_test` or `partial_final_test` rows. Exactly 10 fairness rows have
`test_accessed=true`. Dense-copy, matched low-rank Linear, matched smaller
dense Linear, and shared-only rows baselines now have validation-selected
three-seed final-test rows in `docs/t15_baselines_final_test_summary.md`.
Baseline final-test mean accuracies: dense_copy `0.927867`, smaller_dense `0.927500`,
low_rank `0.921200`, shared_only `0.906267`.
...
A large loss-curve plot is committed at
[`docs/loss_curves/loss_curves.svg`](loss_curves/loss_curves.svg).
It is generated by `docs/render_loss_curves.py` from committed final-comparison
CSVs and real ignored `outputs/**/metrics.jsonl` logs. The manifest
`docs/loss_curves/manifest.csv` records 24 compared final-eval rows and 24
available epoch curves: Stage H families (`baseline_cosine`, `low_lr_cosine`,
`no_balance_cosine`, `wsd`) and T15 baselines (`dense_copy`, `low_rank`,
`shared_only`, `smaller_dense`), three seeds each. No synthetic curve data is used.
...
- Full FFF-replaced student final CIFAR-10 accuracy is established for the four
  Stage H full-student families in `docs/stage_h_all_families_final_test_summary.md`.
  Broader router recipes such as utility-targeted, hard-EM, expert-choice, and
  ST-Gumbel remain validation/layerwise-only.
...
- Dense teacher-copied, matched low-rank Linear, matched smaller dense Linear,
  and shared-only rows baselines have validation-selected three-seed final-test
  evidence in `docs/t15_baselines_final_test_summary.md`.
...
- Remote GitHub SSH auth failed earlier on remote hosts; rsync from `work` was
  ...
  canonical local/remote SHA256 manifests for referenced config and selection
  files before launch.
