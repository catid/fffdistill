...
- The table aggregates committed evidence only: selected dense teacher final test, Stage F
  layerwise FFF validation metrics, T20 route-output single-layer ablations, T19
  optimizer/schedule smoke ablations, T14 KD/final-eval artifacts, official
  `fastfeedforward.FFF` shape smoke, and explicit `not_run` rows for required baselines
  without metrics.
